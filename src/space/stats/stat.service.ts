...
export class StatService implements OnModuleInit {
  ...
  async parseValue(initialKey: string, initialValue: any, entries: any): Promise<string> {
    switch (initialKey) {
      ...
      case 'totalStakeInEra' || 'Return':
        const parsedTotalStakeInEra: string = this.replaceCommaSeparators(initialValue);
        const totalStakeInEra = Number(parsedTotalStakeInEra);
        const lastEraReward = entries.filter((entry) => entry[0] === 'lastEraReward');
        const parsedLastEraReward = this.replaceCommaSeparators(lastEraReward[0][1]);
        const totalStakeInEraReturn = (Number(parsedLastEraReward) / totalStakeInEra) * 100;
        return (Math.round(totalStakeInEraReturn * 1000) / 1000).toString();
      case 'activeEraStart' || 'Era (6 Hours)':
  ...
  async generateColumnOne(worldId: Buffer): Promise<any[]> {
    ...
    const worldDefinition: WorldDefinition = await this.worldDefinitionService.getKusamaDefinition();
