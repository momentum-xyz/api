...
import { favoriteOperator, unfavoriteOperator } from './functions';
...
export class FavoriteController {
  ...
  async addFavorite(
    ...
    // console.log(space);
    // console.log(bytesToUuid(space.spaceTypeId));

    const config = await getKusamaConfig(this.connection);

    if (config.space_types.operator === bytesToUuid(space.spaceTypeId)) {
      // Need to favorite all operator nodes and self space

      await favoriteOperator(this.connection, config, bytesToUuid(user.id), bytesToUuid(space.id));
      return response.status(HttpStatus.CREATED).json({ spaceId: bytesToUuid(space.id), name: space.name });
    }
  ...
  async deleteFavorite(@Param() params, @Req() request: TokenInterface, @Res() response: Response): Promise<Response> {
    ...
    if (config.space_types.operator === bytesToUuid(space.spaceTypeId)) {
      // Need to unfavorite all operator nodes and self space

      await unfavoriteOperator(this.connection, config, bytesToUuid(user.id), bytesToUuid(space.id));
